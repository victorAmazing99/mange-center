...
import { useRoute, useRouter} from "vue-router";
 import tagsTagsView from '@/store/modules/tagsView';
 const Route = useRoute(); //获取路由的值
const Router = useRouter(); 
const tagsView = tagsTagsView();
...
//  const { sys_common_status, sys_job_group } = proxy.useDict("sys_common_status", "sys_job_group");
 ...
 function handleClose() {
   Router.push({ path: '/monitor/index'});
   tagsView.visitedViews.splice(tagsView.visitedViews.findIndex(item => item.path === Route.path), 1)
 }
 /** 搜索按钮操作 */
